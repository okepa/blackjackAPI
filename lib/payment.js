...
class Payment {
  ...
  static createCharge(req, customer) {
    ...
    return new Promise(
      (reject, resolve) => {
        ...
        let charge = stripe.charges.create({
          ...
          currency: "gbp",
          source: customer.id, // obtained with Stripe.js
  ...
  static createCustomer(req, result) {
    return new Promise(
      (reject, resolve) => {
        // Create a Customer:
        console.log("test to see if it reaches create");
        stripe.customers.create({
          email: req.body.email,
          source: result.cardToken,
        }).then(function (customer) {
          // YOUR CODE: Save the customer ID and other info in a database for later.
          User.findByIdAndUpdate({ _id: req.body.id }, { $set: { customer: customer } }, function (err, res) {
            if (err) {
              reject(err);
            } else {
              // console.log("create a customer stripe");
