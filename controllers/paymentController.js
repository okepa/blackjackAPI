...
class PaymentController {
    static getCharge(req, res) {
        Payment.findAndRetrieveToken(req)
            .then((result) => {
                Payment.createCustomer(req, result)
            })
            .then((customer) => {
                console.log("customer for 3rd promise " + customer);
                console.log("for second promise" + req.body.id);   
                Payment.createCharge(req, customer)
            })
            .then((resolveArray) => {
